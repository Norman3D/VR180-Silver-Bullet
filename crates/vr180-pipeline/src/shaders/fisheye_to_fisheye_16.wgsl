// 16-bit (Rgba16Unorm) output variant of fisheye_to_fisheye.wgsl.
// Same math; only the output storage format differs so the 10-bit
// export path stays at full precision through projection.

@group(0) @binding(0) var fisheye_tex: texture_2d<f32>;
@group(0) @binding(1) var fisheye_smp: sampler;
@group(0) @binding(2) var out_tex: texture_storage_2d<rgba16unorm, write>;
