// Fisheye source → stabilized fisheye output WITH per-row rolling-
// shutter correction (one eye). RGBA-input variant for the GUI preview.
//
// Same as `fisheye_to_fisheye.wgsl` plus the per-row direction warp from
// `fisheye_to_hequirect_rs.wgsl`. Only step 1 (output pixel → ray)
// differs from the half-equirect RS shader; steps 2-5 (project_kb →
// row lookup → R_rs → re-project → sample) are identical, because RS
// correction operates on the SOURCE-frame direction after per-frame
// stab, independent of the output parametrisation.
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Equidistant circular-fisheye output parametrisation.
    ...
    // First projection — find which source scanline this pixel maps to.
